...
export default class PrintSheet extends Vue {
    ...
    getRowText(col: SheetColumn) {   
        if (col.rowSettings.bindSource &&  col.rowSettings.bindSource.code !== "sequence") {
            return `{${col.rowSettings.bindSource.name}}`;
        } else if (col.isSequence || col.rowSettings.bindSource &&  col.rowSettings.bindSource.code === "sequence") {
            return '1';
        }
        return '';
    }
