...
export default class AddMethod extends Vue {
  ...
  get bizPropertyTypes() {
    const { xuanrenkongjian, zibiao, StaffMulti,DeptSingle,DeptMulti,StaffSingle,StaffDeptMix } = this.enums;
    if (this.configs.httpType === this.postValue && this.configs.requestBodyType === 1) {
      /* post - json传送时，参数类型不可选：选人控件和子表 */
      return this.paramTypes.filter((item: any) => ![xuanrenkongjian,StaffMulti,DeptSingle,DeptMulti,StaffSingle,StaffDeptMix, zibiao].includes(item.value));
    } if (this.configs.httpType !== this.postValue) {
      /* get 请求时，只能采用查询参数，所以参数类型不能选择子表8 */
      return this.paramTypes.filter((item: any) =>![StaffMulti,DeptSingle,DeptMulti,StaffSingle,StaffDeptMix].includes(item.value));
    }
