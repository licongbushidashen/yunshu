...
currentSheetList
  ...
  */
  ...
  setCurrentDataItems(type:DataItemType, code: string, rowData:any, Datatype: DataRuleType.DataType) {
    ...
    if (type === DataItemType.RelevanceForm || type === DataItemType.RelevanceFormEx) {
    ...
    if (code === 'id' || code === 'parentId') {
      arr = list.filter((item:any) => {
        if (item.propertyType === DataItemType.RelevanceForm || item.propertyType === DataItemType.RelevanceFormEx) {
          if (item.relativeCode !== curModel.code) {
            return false;
          }
        }
        return (item.propertyType === type ||
          ((item.propertyType === DataItemType.RelevanceForm || item.propertyType === DataItemType.RelevanceFormEx) && item.relativeCode === curModel.code)
          )
