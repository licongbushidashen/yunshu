      ...
      <div class="centent">
        ...
        <div class="filters">
          ...
          <a-row style="margin-top: 10px">
            ...
            <a-col :span="5" class="fieldlab" style="margin-left: 5px">对比函数</a-col>
            <a-col :span="4" class="fieldlab" style="margin-left: -5px">值类型</a-col>
            <a-col :span="7" class="fieldlab" style="margin-left: -5px">数据字段值</a-col>
            <a-col :span="1"></a-col>
          </a-row>
          ...
          <a-row v-for="(row, index) in filtersList.child" :key="index" style="margin-top: 5px">
            ...
            <a-col :span="7">
              ...
              <template v-if="row.ruleValueType == 'FIXED' && row.visible">
                ...
                <div style="margin-left:-5px" v-else-if="row.propertyType == 5">
    ...
    if(Array.isArray(value.mainTableConditions)){
      value.mainTableConditions.forEach(item => {
        if(item.targetSchemaDataItem){
          let currentItem:any = this.targetFieldList.main.find(el => el.code === item.targetSchemaDataItem)
          if(currentItem){
            item.targetSchemaDataItemName = currentItem.name
          }
        }
        if(item.currentSchemaDataItem){
          let currentItem:any = item.currentFieldList.find(el => el.code === item.currentSchemaDataItem)
          if(currentItem){
    ...
    switch (targetDataItem.propertyType) {
      ...
      case DataItemType.DropdownMulti:
        this.filtersList[type][index].currentFieldList = this.currentFieldList[
          ...
          return ([DataItemType.ShortText, DataItemType.Radio, DataItemType.Dropdown ].includes(targetDataItem.propertyType) ? [DataItemType.ShortText, DataItemType.Radio, DataItemType.Dropdown ].includes(val.propertyType) : [DataItemType.Checkbox, DataItemType.DropdownMulti ].includes(val.propertyType))
        });
        break;
      case DataItemType.LongText:
      case DataItemType.Number:
      ...
      case DataItemType.DeptSingle:
        this.filtersList[type][index].currentFieldList = this.currentFieldList[
          ...
          return val.propertyType === targetDataItem.propertyType || val.propertyType === DataItemType.StaffDeptMix;
        });
        break;
