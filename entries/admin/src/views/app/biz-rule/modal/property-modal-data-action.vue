  ...
  <section>
    ...
    >
      ...
      <div class="content">
            ...
            <a-col :span="7">
              <template v-if="row.valueType === 'FIXED'">
                <div class="item-box" style="width:215px">
                  ...
                  <!-- 日期 -->
                  <div v-if="getPropertyType(row,'filter') === DataItemTypes.Date && row.ruleConditionType !== 'EP'">
                    <a-date-picker
                      style="width:215px"
                      v-model="row.value"
                  ...
                  <!-- 时间 -->
                  <div
                    v-if="getPropertyType(row,'filter') === DataItemTypes.Time && row.ruleConditionType !== 'EP'"
                  >
                    <a-time-picker
                      style="width:215px"
                      v-model="row.value"
                  ...
                  <!-- 数值 -->
                  <div v-if="getPropertyType(row,'filter') === DataItemTypes.Number && row.ruleConditionType !== 'EP'">
                    <a-input
                      style="width:215px"
                      type="number"
                      v-model="row.value"
                  ...
                  <!-- 逻辑 -->
                  <div v-if="getPropertyType(row,'filter') === DataItemTypes.Logic && row.ruleConditionType !== 'EP'">
                    <a-select
                      style="width:215px"
                      v-model="row.value"
                  ...
                  <div
                    ...
                    v-if="getShowTypeStaff(getPropertyType(row,'filter')) && row.ruleConditionType !== 'EP'"
                  >
                    <staff-selector v-model="row.value" :options="getStaffSelectorOpts(row)"></staff-selector>
                  </div>
                  <!-- 审批意见 -->
                  <div v-if="getPropertyType(row,'filter') === DataItemTypes.ApprovalOpinion && row.ruleConditionType !== 'EP'">
                    <a-select
                      style="width:215px"
                      v-model="row.value"
                      :getPopupContainer="getPopupContainer"
        ...
        >{{actionType === 'createActions' ? '新增动作' : '更新动作'}}</div>
        <div class="filters">
          <a-row>
            <a-col :span="7" class="data-action-modal-content-fieldlab">目标模型数据字段</a-col>
            <a-col :span="5" class="data-action-modal-content-fieldlab">对比函数</a-col>
          ...
          <a-row v-for="(row, index) of assignDataList" :key="index" style="margin-top:5px">
            ...
            <a-col :span="5">
              <a-select :disabled="!row.targetSchemaDataItem" style="width: 153px" v-model="row.ruleActionType" placeholder="请选择">
                <a-select-option
...
export default class AddDataAction extends Vue {
    ...
    if (this.value) {
      ...
      if (this.value[this.actionType].length > 0) {
        ...
        createActions.forEach((res, index) => {
          ...
          let source: any = this.currentDataItem.find(
  ...
  modalOk() {
    ...
    const action = this.assignDataList.map(res => {
      ...
      }else if(res.valueType === 'FIXED' && source.propertyType === DataItemType.Time){
      ...
      if(res.targetSchemaDataItem){
        let currentTargetSchemaDataItem = this.targetModelDataItems.find(el => el.code === res.targetSchemaDataItem)
        obj.targetDataItemName = currentTargetSchemaDataItem.name
      }
      // 记录选中字段的名称
      if(res.currentSchemaDataItem){
        let currentSchemaDataItem = this.filterConditionCurrentDataItemList.find(el => el.code === res.currentSchemaDataItem)
    ...
    switch (source.propertyType) {
      ...
      case DataItemType.Dropdown:
        return sourceData.filter((val) => {
          return val.propertyType === DataItemType.ShortText || val.propertyType === DataItemType.Radio || val.propertyType === DataItemType.Dropdown ;
        });
        break
      case DataItemType.Checkbox:
      case DataItemType.DropdownMulti:
        return sourceData.filter((val) => {
          return val.propertyType === DataItemType.Checkbox || val.propertyType === DataItemType.DropdownMulti;
        });
        break
      case DataItemType.LongText:
      case DataItemType.Number:
      ...
      case DataItemType.StaffDeptMix:
        return sourceData.filter((val) => {
          return row.currentSchemaDataItem === val.code || val.propertyType === source.propertyType && val.code !== SystemDataItemCode.Modifier;
        });
        break;
      case DataItemType.DeptMulti:
      case DataItemType.DeptSingle:
      ...
      case DataItemType.StaffMulti:
        return sourceData.filter((val) => {
          return (val.propertyType === source.propertyType || val.propertyType === DataItemType.StaffDeptMix)  && val.code !== SystemDataItemCode.Modifier;
        });
        break;
      case DataItemType.RelevanceForm:
        let idArr: Array<any> = [];
