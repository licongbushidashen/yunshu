...
export default class PropertyFilterCondition extends Mixins(PropertyComponent) {
  ...
  modalOk() {
    ...
    Object.getOwnPropertyNames(value.itemList).length === 0
      ? (value.itemList = [])
      : value.itemList;
    ...
    if(Array.isArray(value.itemList)){
      value.itemList.forEach((item:any) => {
        if(item.targetSchemaDataItem){ //
          let current = this.targetFieldList.main.find(el => el.code === item.targetSchemaDataItem)
          if(current){
            item.targetSchemaDataItemName = current.name
          }
        }
        if(item.value){
          let current = item.currentFieldList.find(el => el.code === item.value)
          if(current){
