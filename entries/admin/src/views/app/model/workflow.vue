...
export default class Workflow extends Vue {
  ...
  notifyDataForamt(data:any) {
    ...
    array.forEach((item:any) => {
      if( typeof item === 'string'){
        item = item.trim();
        if (item) {
          const flag:boolean =Array.isArray(this.WorkflowListItem) ? (this.WorkflowListItem.findIndex((dataItem:any) => dataItem.id === item) > -1) :false;
  ...
  setWorkflowDataForAjax() {
    ...
    _activities.forEach((item: any) => {
      ...
      if (item.activityType === 'PARTICIPANT' || item.activityType === 'SUB_INSTANCE') {
        item.todoDataItem.summary = (item && item.todoDataItem && item.todoDataItem.summary && item.todoDataItem.summary[0] && item.todoDataItem.summary[0].hasOwnProperty('isDataItem') && item.todoDataItem.summary[0]['isDataItem'] === 2) ? this.summaryDataForamt(item.todoDataItem.summary) : this.notifyDataForamt(item.todoDataItem.summary);
        item.todoDataItem.title = this.notifyDataForamt(item.todoDataItem.title);
        item.todoDataItems && item.todoDataItems.length && item.todoDataItems.forEach(v => {
          if(v.belong === 'email'){
    ...
    if (res.errcode === 0) {
      if (res.data && res.data.length > 0) {
        ...
        if (this.validateOrPublish === 'validate') {
          ...
          this.$message.success(`版本（Version ${res.errmsg}）流程发布成功！`, 1);
        }
      }
  ...
  */
  ...
  @Provide()
  toWorkflowSetting() {
    return this.openWorkflowSetting;
  }
  // 打开流程设置页面
  openWorkflowSetting() {
    ...
    this.deleteWorkflow(params).then((res: any) => {
      if (res.errcode === 0) {
        if (this.$route.name === 'workflowDesign' && code === this.$route.params.workflowCode) {
          this.notValid = true;
          this.$router.push(`/apps/model/${this.appCode}/${this.bizSchemaCode}/data/dataModel`).catch((err: any) => {err});
        }
        this.$message.success(res.errmsg);
        this.getWorkflows();
