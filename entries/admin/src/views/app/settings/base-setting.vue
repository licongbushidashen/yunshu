  ...
  <div class="base-settings">
    ...
    <div class="base-settings__form">
      ...
      <div class="base-settings__form-item">
        ...
        <div>
          <a-select :filter-option="filterOption" showSearch v-model="form.groupId" style="width: 268px;">
            <a-select-option
...
export default class BaseSetting extends Vue {
  ...
  async save() {
    if (this.form.code && this.form.name) {
      if (this.file) {
        this.uploading = true;
        const res: AppSettings.uploadResponse = await appBaseApi.uploadFile({ file: this.file, rewriteFilename: true });
        if(res.errcode !== 0){
  ...
  beforeUpload(file: File) {
    ...
    if (!isPic) {
      this.$message.error(this.$t('languages.appSettings.PlzUploadImageFile') as string);
      return false;
    }
    if (!isLimitSize) {
      this.$message.error(this.$t('languages.appSettings.ImageSizeTip', { size: '100KB' }) as string);
      return false;
    }
  ...
  mounted() {
    const {
      code, name, enabled, logoUrlId, logoUrl, content, name_i18n, groupId
    } = this.appInfo;

    this.form = {
      code, name, enabled, logoUrlId, groupId
    };
