<template>
  <div class="biz-model-group">
    <div class="biz-model-group-head" v-if="itemData.children">
      ...
      <span class="biz-model-group-head-num" v-show="itemData.num">{{ `${$t('languages.Apps.Model')}*${ itemData.num}` }}</span>
      <a-popover
        placement="rightTop"
        trigger="click"
        :visible="isShowPop"
      ...
      <span class="biz-model-group-head-empty" v-if="!itemData.num">
...
export default class BizModelGroup extends Vue {
  ...
  get allItemData(){ // 多级树数据结构拉平，只展示最里层节点
  ...
  */
  popMenu(item: any) {
    ...
    this.validFolders(validParams).then((res: Common.Data) => {
      if (res.errcode) {
        /* 删除分组 —— 分组下有模型 */
        let contents: any = this.$t('languages.Apps.GroupHasModelCannotBeDelete');
