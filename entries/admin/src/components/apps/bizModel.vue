...
export default class BizModel extends Vue {
  ...
  // }
  get visiable(){
    console.log(!this.searchKey  ,this.itemData.name.indexOf(this.searchKey) !== -1)
  ...
  deleteReportConfirm(item: any) {
    const vm: any = this;
    this.$confirm({
      title: (this as any).$t("languages.Apps.ConfirmDelete", { name: item.name }),
  ...
  async handleExport(item: any) {
    ...
    if (res.errcode && res.errcode !== 0) {
      this.$message.error((this as any).$t("languages.Apps.ExportError") as string);
    } else {
