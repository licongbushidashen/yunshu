  ...
  <div class="section-bar">
    ...
    <span class="bar-right">
      ...
      <span
        ...
        :key="index"
      >
        <a-button v-if="!el.children" type="default">{{ isChinese ? el.name : el.name_i18n[$i18n.locale] }}</a-button>
        <a-dropdown v-else>
          <a-menu slot="overlay">
            <a-menu-item :key="index" v-for="(el, index) in el.children">
              {{ isChinese ? el.name : el.name_i18n[$i18n.locale] }}
            </a-menu-item>
          </a-menu>
          <a-button> {{ isChinese ? el.name : el.name_i18n[$i18n.locale] }} <a-icon type="down" /> </a-button>
        </a-dropdown>
      </span>
    </span>
    <!-- 弹窗: 过滤条件预览 -->
