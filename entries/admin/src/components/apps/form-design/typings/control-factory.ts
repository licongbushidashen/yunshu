...
export function autoLayout(
  ...
  items = items.filter((x) => x.published && !x.isSystem && x.code !== "parentId");
  ...
  for (const item of items) {
    ...
    if (isFullRow(control.type) || layout.length === 0) {
      ...
      const idx = layout.length - 2;
      if (layout[idx].length === 2 || (map[layout[idx][0]] && isFullRow(map[layout[idx][0]].type))) {
        layout.push([control.key]);
      } else {
