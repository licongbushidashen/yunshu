      ...
      <div class="filter-wrap" v-show="isShowFilterBox">
        <div class="content-box" >
          <query-form
            ...
            :cols="cols"
            :class="{'pointerEventsNone': isPreview}"
          ></query-form>
        </div>
    ...
    <div class="table-wrap">
      ...
      >
        ...
        <template  v-for="(item, index) in columns"
          ...
          <span :key="index">{{ getLabelContent(column, rowIndex, index) }}</span>
        </template>
      </sheet>
    </div>
  </div>
...
export default class ListSelector extends Vue {
  ...
  initColumns() {
    ...
    const columns: any[] = this.data.queryColumns.map((c: any,index: number) => ({
      title: c.name,
