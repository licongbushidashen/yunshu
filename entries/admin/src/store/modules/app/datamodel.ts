...
// 
  ...
  setDataItemList(state: any, dataItem: Array<Apps.Datamodel.DataItem>) {
    ...
    let list = dataItem.filter((x: any) => x.type !== DataItemType.ApprovalOpinion).map(convert);
    list.forEach(i => {
      if (i && i.properties) {
    ...
    for (let i = 0; i < dataLength; i += 1) {
      ...
      if (dataItem[i].subSchema) {
        console.log(dataItem[i].subSchema, 'dataItem[i].children')
        dataItem[i].children = dataItem[i].subSchema.properties.filter((res: any) => {
    ...
    console.log(dataChange,'ddddddddddddddddd',state.summaryList)
...
const actions = {
  ...
  async getDataItemList({commit, state}: any, param?: any) { // 获取数据项数据
    let params: Apps.Datamodel.DataItemListParams = {schemaCode: state.bizSchemaCode};
