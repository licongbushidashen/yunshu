    ...
    <div class="table-box" v-if="isShowTableBox">
      ...
      <div class="table" ref="table">
        ...
        >
          ...
          <span slot="instanceNameTitle">任务名称</span>
          ...
          <template slot="stayTime" slot-scope="{ text, record }">
            <span :class="record.isRead ? 'gray' : ''" :title="record.time"
              >{{ record.time
              }}<span
                v-if="
      ...
      <div class="pagination-box">
        <a-pagination
          ...
          "
          :pageSize="pageSize == 20 ? 15 : pageSize"
    ...
    >
      ...
      <template slot="content">
        ...
        <div class="transfer-list">
          ...
          <div>
            <a-textarea placeholder="请输入" :rows="4" v-model="remark" />
          </div>
        </div>
      </template>
    </customPop>
...
import BatchInfo from "@cloudpivot/flow-center/src/components/pc/components/modals/batch-extra-info.vue";
import BatchFailList from "@cloudpivot/flow-center/src/components/pc/components/modals/batch-fail-list.vue";
...
import * as WorkflowCenterHelper from "@cloudpivot/flow-center/src/components/pc/helper/helper";
...
export default class MyUnfinishedWorkItem extends mixins(WorkItemMixin) {
  ...
  openDetail(obj: any) {
    ...
    const url = `/form/detail?workitemId=${obj.id}&workflowInstanceId=${
      ...
      location.search}&workitemType=unfinishedWorkitem`;
  ...
  startWorkflow() {
    ...
    if (this.rootAdmin || isAdmin) {
      ...
      this.$message.warn(this.$t(
        "cloudpivot.flowCenter.pc.adminNewWorkflowErr"
      ) as ConfigType);
      return;
    }
