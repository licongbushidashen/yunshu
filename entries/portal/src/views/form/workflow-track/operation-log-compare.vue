<template>
  <div class="operation-log-compare">
    ...
    <div class="c-box">
      <a-tabs>
        <a-tab-pane :tab="$t('languages.common.postEdit')" key="1">
          <pc-form-renderer
        ...
        <a-tab-pane :tab="$t('languages.common.preEdit')" key="2">
          <pc-form-renderer
            :class="{'has-form-border': borderMode}"
            v-if="preControls"
            ref="formRenderer"
          ...
          ></pc-form-renderer>
          <div v-else>{{$t('languages.common.noHistoryRec')}}</div>
        </a-tab-pane>
      </a-tabs>
    </div>
  </div>
          ...
          if (typeof curr.value === 'string' || typeof curr.value === 'number' || typeof curr.value === 'boolean') {
            ...
            switch(cType) {
              ...
              case  FormControlType.DepartmentMultiSelector:
                  if (curr.value.length === pre.value.length && curr.value.length) {
                    let cid = curr.value.reduce((c,v) =>{return `${c}${v.id}`}, '')
                    let rid = pre.value.reduce((c,v) => {return `${c}${v.id}`},'')
                    if (cid !== rid) {
                      curr.diff = true
                      pre.diff = true
                    }
            ...
            if((curr_col[k]&&!pre_col[k]) || (!curr_col[k]&&pre_col[k])) {// 两个子表, 列不对称
              ...
              if(Array.isArray(curr_col[k])) {
                ...
                if (typeof curr_col[k][0] === 'string') {
                  ...
                  let cid = curr_col[k].reduce((c,v) =>{return `${c}${v.id}`}, '')
                  let rid = pre_col[k].reduce((c,v) => {return `${c}${v.id}`},'')
                  if (cid !== rid) {
                    sheet_diff_key[k] = true
                  }
