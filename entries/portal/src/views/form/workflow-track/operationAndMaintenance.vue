            ...
            <div class="list" style="position: relative; padding-bottom: 10px;">
              ...
              >
                {{
                  (currentNode &&
                    ...
                    currentNode.participants.map(el => el.name).join("、")) ||
                    ""
                }}
              </div>
              <template
              ...
              >
                <span
                  @click="showAll = false"
                  v-if="showAll"
                  class="icon-up icon aufontAll h-icon-all-double-up-o"
                ></span>
                <span
                  @click="showAll = true"
                  v-else
                  class="icon-up icon aufontAll h-icon-all-double-down-o"
                ></span>
              </template>
            </div>
          </div>
            ...
            <a-popover placement="bottom" trigger="click">
              ...
              <template slot="content">
                <div class="to-work-handover" @click="toWorkHandover('dep')">
                  <p>
                    组织机构<i class="icon aufontAll h-icon-all-finder-o"></i>
                  </p>
                ...
                <div class="to-work-handover" @click="toWorkHandover('role')">
                  <p>
                    组织角色<i class="icon aufontAll h-icon-all-finder-o"></i>
                  </p>
            ...
            <a-radio-group v-model="manage" @change="onManageChange">
              <div class="radio-wrapper">
                ...
                <a-radio
                  ...
                  >激活其他节点</a-radio
                >
                <a-radio v-if="!isWORKFLOW_ADMIN" value="xiugai"
                  >修改流程模板</a-radio
                >
                <a-radio v-if="isShow('MODIFY_OWNER')" value="MODIFY_OWNER"
                  >修改拥有者</a-radio
                >
              </div>
            </a-radio-group>
            ...
            <div class="multiplexing">
              ...
              <a-popover placement="top" overlayClassName="black-tooltip">
                <template slot="content">
                  ...
                  <p>
                    2.“同类型”指同一模型、同一节点、同一状态、同一处理人的流程
                  </p>
                </template>
                <i
          ...
          <div class="row-item">
            ...
            <div>
              <span class="show-other" @click="showNoParticipantsList = true"
                >【{{ data.workflowName }}】无参与者流程{{ total }}条</span
              >
            </div>
          </div>
        </template>
...
export default class batchOperationAndMaintenance extends Vue {
  ...
  toModifyWorkflowTemplate() {
    ...
    // 去修改流程模板
    this.winOpen(
      `/admin/#/apps/model/${params.appCode}/${
        params.schemaCode
      }/workflowDesign/${params.workflowCode}`,
      "_blank"
    );
  }
  ...
  popOK() {
    ...
    if (this.manage === "ACTIVATE_ACTIVITY") {
      ...
      this.tipsTitle = "删除";
      this.tipsContent =
        '流程删除后该流程所有数据将删除，<span style="color: red;">且删除后不可恢复</span>，确定要删除？';
      this.tipsModalShow = true;
