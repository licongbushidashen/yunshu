...
export default {
   ...
   * 构建打印表单
   */
  async printFormCode(vm: any, { arr, changeArr, formControls }: any, data: any) {
    vm.formdata = arr
    vm.formControls = formControls || []

    vm.printHtmlCloseLoading = (vm.$message as any).loading(vm.$t("cloudpivot.list.pc.loading").toString(), 0)
    if (!data.draftAttributesJson || !JSON.parse(data.draftAttributesJson).length) {
      vm.$message.warning("打印模板内容为空！", 3)
      return
    }
    vm.formObj = arr
    // @ts-ignore
    vm.draftAttributesJson = replaceValueData(env, JSON.parse(data.draftAttributesJson) || [], "real", vm)
    ...
    vm.tempPrintPageSettings === "" ? (vm.tempPrintPageSettings = pageSettings) : vm.tempPrintPageSettings
                  ...
                  if (map[key]) {
                    if (
                      ...
                      if (
                        ...
                        row[key] && row[key][row[key]["displayCode"]] ? (str = row[key][row[key]["displayCode"]]) : str
                        // 逻辑
                        ...
                        if (row[key] && row[key].propertyType === 10 && str) {
                          ...
                          if (
                            ...
                            // @ts-ignore
                            str = `${obj.provinceName || ""}${obj.cityName || ""}${obj.districtName || ""}${obj.address || ""}`
                          }
                        }
                        // 选人控件处理
                      ...
                      // 手写签名
                        ...
                        let signature: any = {}
                        Array.isArray(row[key]) && row[key].length > 0 ? (signature = row[key][0]) : (signature = [])
                    ...
                    if (Array.isArray(val)) {
                      ...
                      if (val.name) {
                        ...
                        row[key] = `${val.provinceName} ${val.cityName} ${val.districtName} ${val.address}`
                      } else if (val.address) {
                        row[key] = val.address
                      } else {
                        row[key] = ""
      ...
      for (let j = 0; j < page.length; j++) {
        ...
        if (ele.eleType === "column" && ele.rightValue && ele.rightValue.code) {
          ...
          if (ctrl.type === schema.FormControlType.Textarea) {
            ...
            // 兼容富文本打印
            if (ctrl.options && ctrl.options.textAreaType && ctrl.options.textAreaType === "current") {
  ...
  printPageAlgorithm(vm: any, value: any, page: any, p: any, eleIndex: any) {
    console.log("PcFormDetail -> printPageAlgorithm -> vm.collectorTempPrintContainer", vm.collectorTempPrintContainer)
    let eleInfo = vm.collectorTempPrintContainer[page][p][eleIndex]
    if (eleInfo.calcDiff) {
      let [aIndex, pIndex, sIndex] = this._findPrintEleIndex(eleInfo.id, value, vm) // 根据id获取元素当前位置, 因为在for中会操作draftAttributesJson
      ...
      if (ele.top + eleInfo.eleContentHeight + eleInfo.pageUpToPx > eleInfo.PageHeightPx) {
        newPageNum = Math.ceil(
          Math.abs(eleInfo.eleContentHeight - (eleInfo.PageHeightPx - ele.top - eleInfo.pageUpToPx)) /
      ...
      // 获取那些元素在 变化高度元素下面.
      console.log(vm.draftAttributesJsonSet[aIndex][pIndex].length > sIndex + 1, "dd", vm.draftAttributesJsonSet[aIndex][pIndex])
      ...
      if (vm.draftAttributesJsonSet[aIndex][pIndex].length > sIndex + 1) {
        ...
        for (let i = sIndex + 1; i < l; i++) {
          ...
          if (!(tItem.left + tItemContentWidth <= eleMinWidth || tItem.left >= eleMaxWidth) || diffTop >= 0) {
        ...
        if (eleSiblingIndex.length && newPageNum) {
          for (let i = 0; i < eleSiblingIndex.length; i++) {
            let [s] = vm.draftAttributesJsonSet[aIndex][pIndex].splice(eleSiblingIndex[i] - i, 1)
            eleSibling.push(s)
          }
        } else if (passivity_eleSiblingIndex.length && passivity_newPageNum) {
          for (let i = 0; i < passivity_eleSiblingIndex.length; i++) {
            let [s] = vm.draftAttributesJsonSet[aIndex][pIndex].splice(passivity_eleSiblingIndex[i] - i, 1)
    ...
    console.log("PcFormDetail -> printPageAlgorithm -> this.draftAttributesJson", vm.draftAttributesJsonSet)
            ...
            formData[key] = logInfoList
              ...
              .map(item => {
                return {
                  ...
                  name: item.name,
                  operating: workflowActionStatus[item.approvalActionStatus.toString()]
                    ? vm.$t(`cloudpivot.flow.pc.WorkflowActionStatus.${workflowActionStatus[item.approvalActionStatus.toString()]}`)
                    : "--",
