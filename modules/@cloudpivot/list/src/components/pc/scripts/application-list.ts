...
import FormMarking from "../components/form-marking.vue"
...
@Component({
  name: "app-list",
  components: {
    sheet: () => import(/* webpackChunkName: "sheet" */ "@cloudpivot/form/src/renderer/components/pc/form-sheet/sheet.vue"),
    ImportErrModal,
...
export default class AppList extends Vue {
  ...
  caculateColWidth(columns: any, parentCode?: string): number {
    ...
    if (records) {
      ...
      const item: AllTypes.Record = recordJson.value.find((d: AllTypes.Record) => Object.keys(d)[0] === code) as AllTypes.Record
      if (item) {
        colWidth = item[code]
      } else {
  ...
  async actionClick(action: listParams.QueryActions, e: any) {
    ...
    // onPreAction 执行前
    if (this.eventHooksHandler && (await this.eventHooksHandler.exec("onPreAction", action)) === false) {
      return
    }
      ...
      if (Array.isArray(res.data.queryConditions)) {
        res.data.queryConditions.forEach((condition: any) => {
          ...
          if (condition.propertyType === 3 && condition.dateType) {
            const date = Helper.setDateByDateType(condition.dateType, this.getFormat(condition.displayFormat))
            condition.startValue = date[0] ? moment(date[0], this.getFormat(condition.displayFormat)) : date[0]
            condition.endValue = date[1] ? moment(date[1], this.getFormat(condition.displayFormat)) : date[1]
          }
  ...
  setFilterData(data: any) {
            ...
            switch (query.propertyType) {
              case DataItemType.RelevanceForm:
                ...
                if (value && relevanceDataList && relevanceDataList.length > 0) {
                  ...
                  let val: any
                  rele.relativePropertyCode = rele.relativePropertyCode ? rele.relativePropertyCode : "name"
    ...
    filterData = filterData.map(item => {
      let dataItem = this.queryConditions.find(i => {
      ...
      if (item.propertyValue && displayFormat && displayFormat === "5") {
        let propertyValuesplit0 = item.propertyValue.split(";")[0]
        let propertyValuesplit1 = item.propertyValue.split(";")[1]
        item.propertyValue = `${propertyValuesplit0.substring(0, 4)};${propertyValuesplit1.substring(0, 4)}`
      }
      return item
    })
  ...
  async getQueryList(type?: string, ...otherParams) {
    ...
    if ((isCustomRequest && customRequestMode !== "combine") || res.errcode === 0) {
      ...
      if (!hasData || type === "search" || type === "pageChange" || type === "reload" || type === "append") {
        if (this.eventHooksHandler) {
  ...
  listDataFormatter(dataOrigin: any, dataSource: any, sourceColumns: any, isChildSheet?: any) {
    dataOrigin.forEach((item: any, index: number) => {
      ...
      Object.entries(itemData).forEach((data: any, i: number) => {
        ...
        const column: any = sourceColumns.find((c: any) => c.dataIndex === key || c.propertyAlias === key)
        // 键值
        if (value && typeof value === "object") {
          if (Array.isArray(value)) {
            // MARK: 如果是附件、子表, 返回全部, 而不只是名字;
            const ignoreType = [6, 8]
            obj[key] = column && ignoreType.includes(column.propertyType) ? value : value.map(x => x.name || "").join()
          } else {
            ...
            if (Array.isArray(value[code])) {
              ...
              if (value.propertyType === 10) {
                ...
                if (addressObj) {
                  ...
                  addressObj.districtName ? (obj[key] += addressObj.districtName) : (obj[key] += "")
          ...
          if (value[value["displayCode"]] && value[value["displayCode"]] !== true) {
      ...
      if (this.modelType === "TREE") {
        ...
        if (item.data) {
          if (item.data.t__children_) {
            this.listDataFormatter(item.data.t__children_, obj.t__children_, sourceColumns)
          }
        }
      }
    })
  ...
  async goForm(column: any, rowIndex: number) {
    ...
    // 否则返回一个字符串
      ...
      let questionPosition = location.href.indexOf("?")
      let search = location.href.substring(questionPosition) + "&iframeAction=detail"
  ...
  async goTreeForm(column: any, rowIndexid: number) {
    ...
    // 否则返回一个字符串
      ...
      const url = this.isDingTalk ? `${res}&return=${location.pathname}` : `${res}&return=${location.pathname + encodeURIComponent(search)}`
      if (platform.IS_DINGTALK) {
        // 缓存查询条件
        const { curPage, filterData, queryFormValues, pageSize } = this
        const cacheData: any = {
