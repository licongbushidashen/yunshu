<template>
  <span class="filter-card mr16" v-if="source.length === 1">
    <template v-if="sourceSingle.type === 4">
      <span
        class="item-title"
        :title="isChinese ? sourceSingle.name : JSON.parse(sourceSingle.name_i18n)[$i18n.locale]"
      >{{ isChinese ? sourceSingle.name : JSON.parse(sourceSingle.name_i18n)[$i18n.locale] }}:</span>
      <img class="logic-img" v-if="sourceSingle.content" src="./images/on.png" alt />
      <img class="logic-img" v-else src="./images/off.png" alt />
      <i class="icon aufontAll h-icon-all-close" @click.stop="clear" style="vertical-align: unset"></i>
    </template>

    <template v-else>
      <span
        class="item-title-all"
        :title="(isChinese ? sourceSingle.name : JSON.parse(sourceSingle.name_i18n)[$i18n.locale]) + ': ' + contentHandler(sourceSingle.type)"
      >{{ isChinese ? sourceSingle.name : JSON.parse(sourceSingle.name_i18n)[$i18n.locale] }}: {{ contentHandler(sourceSingle.type) }}</span>
