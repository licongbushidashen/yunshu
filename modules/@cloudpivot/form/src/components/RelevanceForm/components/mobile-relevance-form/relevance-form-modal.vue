<template>
  <div v-show="visible">
    ...
    <div v-show="canAdd" class="home-add" @click="openRelevanced" v-if="control.key !== 'parentId'">
      <img src="./add.png" />
    </div>
  </div>
</template>
...
export default class RelevanceFormModal extends RelevanceFormControl {
  ...
  superFetch(page: number, pageSize: number, value: string, formState: any ,view:any,) {
    let query: any = [];
    if (formState) {
      query = [formState];
    } else {
