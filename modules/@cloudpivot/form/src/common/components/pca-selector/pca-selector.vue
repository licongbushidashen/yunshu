<template>
  <div class="pca-selector">
    ...
    <div
      ...
      v-show="isFocus"
      :style="{'top': `${inReport?'':position.top+'px'}`, 'left': `${inReport?'':position.left+'px'}`}"
      @click.stop="isFocus = true"
    >
      <a-button v-if="prevConfirm" @click.stop="confirmpca" class="pca-selector-confirm" type="primary">确定</a-button>
      <a-tabs
        @change="setPosition"
        v-model="activeTab"
      >
        ...
        >
          <div class="city">
            <label v-if="showEmpty">
              <span @click.stop="emptyClick('c')">请选择</span>
            </label>
            <label
        ...
        >
          <div class="area">
            <label v-if="showEmpty">
              <span @click.stop="emptyClick('a')">请选择</span>
            </label>
            <label
...
export default class PcaSelector extends Vue {
  ...
  getPcaBaseData() {
    if (!this._provinces || this._provinces.length === 0) {
      ...
      return import(/* webpackChunkName: "pca" */"@cloudpivot/common/src/pca/pca.min").then(mod => {
