...
export function checkParam(claim: Formula.Param, incoming: Formula.Param) {
  const paramType = checkParamType(claim.paramType as Formula.ParamType, incoming.paramType as Formula.ParamType)
...
export function checkContainsParam(claim: Formula.Param, incoming: Formula.Param) {
  ...
  const paramTypeInfo = paramType
    ? ["", ""]
    : [`类型：文本、单选框、复选框、下拉单选框、下拉多选框`, `类型：${getParamTypeValue(incoming.paramType as Formula.ParamType)}`]
  const sourceTypeInfo = sourceType ? ["", ""] : ["来源：字段", "来源：非字段"]
...
export function checkDateString(claim: Formula.Param, incoming: Formula.Param) {
  ...
  const paramTypeInfo = paramType ? ["", ""] : [`类型：文本、日期`, `类型：${getParamTypeValue(incoming.paramType as Formula.ParamType)}`]
  const sourceTypeInfo = sourceType ? ["", ""] : ["来源：字段", "来源：非字段"]
...
export function checkFieldParamFromRelevance(identifier: Formula.Identifier, params: Formula.Param[]) {
  const _params = params.filter(
    param => param.sourceType === Formula.SourceType.FIELD && (param as Formula.FieldParam).fieldType === Formula.FieldType.RELEVANCE
  )
  if (_params.length > 0 && !extraRelevanceFuncs.includes(identifier.name)) {
    throw generateError(
      `关联数据字段${_params.map(_param => getFieldName(_param)).join(", ")}只允许在函数 ${extraRelevanceFuncs
        .map(func => `${func}()`)
        .join("，")}中使用，不允许在${
...
export function checkFieldParamFromSublist(identifier: Formula.Identifier, params: Formula.Param[]) {
  const _params = params.filter(
    param => param.sourceType === Formula.SourceType.FIELD && (param as Formula.FieldParam).fieldType === Formula.FieldType.SUBLIST
  )
  if (_params.length > 0 && !extraSublistFuncs.includes(identifier.name)) {
    throw generateError(
      `明细表字段${_params.map(_param => getFieldName(_param)).join(", ")}只允许在函数 ${extraSublistFuncs
        .map(func => `${func}()`)
        .join("，")}中使用，不允许在${
        identifier.type === Formula.IdentifierType.FUNC ? `函数 ${identifier.name}() 中` : `操作符 "${identifier.name}" `
      }使用`
...
export function validateFormulaNested(
  ...
  for (const _fieldId of fieldIds) {
    ...
    if (_formulaIds.includes(_fieldId)) {
      ...
      err = generateError(
        `公式中存在公式字段${subFormulaIds.map(id => `${getFieldName(fields[id])},`).join(" => ")}嵌套了公式字段${name}，不允许公式循环嵌套`
      )
      if (err) {
        break
      }
