...
> extends Vue {
  ...
  recordDelFile(delFile: any) {
    if(this.$root.$store.state.delFile) {
      // @ts-ignore
      this.$root.$store.state.delFile.push({ name: delFile.response ? delFile.response.data.name : delFile.name, refId: delFile.response ? delFile.response.data.refId : delFile.refId })
    } else {
      // @ts-ignore
      this.$root.$store.state.delFile = [{ name: delFile.response ? delFile.response.data.name : delFile.name, refId: delFile.response ? delFile.response.data.refId : delFile.refId }]
    }
  }
      ...
      if (dataLinkage) {
        ...
        condArr.forEach(x => {
          ...
          if (controller && controller.valueChange) {
            // 有默认值且默认值不为空时触发默认值
            if (this.getIsNew() && controller.hasValue && controller.value == controller.options.value && controller.value !== null && controller.value !== undefined && controller.value !== "") {
              setTimeout(() => {
                let filterArr: any[] = [];
                let extraArr: any[] = [];
        ...
        relevantDataItems.forEach((formulaItemKey: string) => {
          ...
          if (idx > -1) {
            ...
            if (formulaStr.indexOf('COUNT') > -1 && formulaStr.indexOf('COUNTIF') === -1) {
              // 待优化
              // 这里根据公式中包含COUNT 判断是COUNT函数 直接订阅rowChange, 其他订阅cellChange
              if (controller && controller.rowChange) {
                const optimizationRowChange = controller.rowChange.pipe(debounceTime(500));
                optimizationRowChange.subscribe(() => {
                  if (!ctrl.readonly) {
                    setTimeout(() => {
                      let bizRuleCalculate: any = {};
                        ...
                        if (Array.isArray(relevantDataItems)) {
                          ...
                          formApi.getComputeFormulaVal(bizRuleCalculate)
                          .then((res: any) => {
                              ...
                              if (dataItem) {
                                switch(dataItem.propertyType) {
                                  ...
                                  case DataItemType.Number:
                                    ctrl.value = res.data ? Number(res.data) : '';
                                    break;
                                  default:
                                    ctrl.value = res.data;
                                    break;
                        ...
                        if (Array.isArray(relevantDataItems)) {
                          relevantDataItems.forEach((formulaInnerKey: string) => {
                            ...
                            if (idx > -1) {
                              ...
                              let isAggregateFunc = formulaStr.indexOf("SUM") > -1 || formulaStr.indexOf("AVG") > -1 || formulaStr.indexOf("MAX") > -1 || formulaStr.indexOf("MIN") > -1 || formulaStr.indexOf("COUNTIF") > -1;
                              let tmpVal = null;
                              if (controlKey === innerControlKey) {
                                ...
                                relevantCtrl = this.findController(sheetKey) as any;
                              if (relevantCtrl) {
                                tmpVal = isAggregateFunc ? relevantCtrl.value.map((item: any) => item[innerControlKey]) : relevantCtrl.value.map((item: any) => item[innerControlKey])[change.rowIndex];
                                }
                              }
  ...
  emitControlOnChange(change: ControlValueChange) {
    ...
    if (this.ctrl.inSheet) { // 如果在子表内触发了事件 则要标记当前行状态
