...
import { ControlHelper, UploadStatus } from "@cloudpivot/form/src/renderer/controls"
...
export abstract class RelevanceFormControl extends BaseControl<typings.RelevanceFormOptions> {
  ...
  async getRelativePropertyCode() {
    ...
    let field = dataModal && dataModal.relativePropertyCode ? dataModal.relativePropertyCode : this.control.options.displayField || "name"
    return field
  }
  get getDisplayField() {
    return this.control.options.relativePropertyCode || this.control.options.displayField || "name"
  }
  ...
  fetch(
    ...
    //经产品测试沟通，关联单选树形控件显示样式为树形时不需要数据限定范围
    if (this.controlOpts.showStyle !== "tree") {
      // 添加关联表单添加的过滤条件
      this.searchFormula.forEach((el: any) => {
        params.push({
          code: el.propertyCode,
          type: el.propertyType,
          value: typeof el.value === "string" ? el.value : JSON.stringify(el.value)
        })
      })
    }
    ...
    return RelevanceFormControl.service.search(schemaCode, queryCode, params, cols, page, pageSize, queryData, view)
  }

  get searchFormula() {
    // @ts-ignore
    let _searchFormula = this.controlOpts && this.controlOpts.searchFormula
    return _searchFormula ? JSON.parse(_searchFormula).conditions : []
  }

  async convertForMappings(value: any) {
    const _ctrl = this.ctrl as any
    if (!_ctrl.options || !_ctrl.options.mappings || !this.controlOpts.schemaCode) {
      return value
    }
    const dataitems = await RelevanceFormControl.service.dataitemsOf(this.controlOpts.schemaCode)
    for (let a in value) {
      if (value[a] && value[a]._isAMomentObject) {
        // console.log(value[a]._f);
        value[a] = value[a].format(value[a]._f)
      }
      if (value[a] && Object.prototype.toString.call(value[a]) === "[object Date]") {
    ...
    for (const key in _ctrl.options.mappings) {
      ...
      if (idx === -1) {
        ...
        const parent = formControls.find(c => c.key === parentKey) as any as typings.FormSheet
      ...
      const dataitem = dataitems.find((d: typings.DataItemLike) => d.code === key)
      if (
        !dataitem ||
        !targetItem ||
        (targetItem.propertyType !== dataitem.propertyType && ![0, 1, 12, 13, 14, 15].includes(targetItem.propertyType)) // form-control-type类型与data-item-type类型不相等，可能会有问题
      ) {
      ...
      if (control.type === typings.FormControlType.Sheet) {
        ...
        if (Array.isArray(rows) && rows.length > 0 && columns.length > 0) {
          ...
          for (const col of columns) {
            const subItem = subSchema ? subSchema.properties.find(p => p.code === col.key) : null

            const match = subItem && subItem.propertyType === ControlHelper.mapToDataItemType(col.type)
                ...
                if (match) {
                  if (key === "sequenceStatus") {
                    ...
                    if (col.type === typings.FormControlType.Image || col.type === typings.FormControlType.Attachment) {
                      ...
                      row[col.key] = FormControlValueService.convert(col.type, row[col.key], col.options.multi)
                    }
                  }
                } else {
                  row[col.key] = FormControlValueService.defaultValueOf(col.type)
                }
              }
            }
          }
      ...
      } else if (control.type === typings.FormControlType.Attachment || control.type === typings.FormControlType.Image) {
        ...
        if (key === "sequenceStatus") {
          ...
          item[key] = FormControlValueService.convert(control.type, item[key], control.options.multi)
        }
      }
    }
    if (Array.isArray(item)) {
  ...
  getParams(isMobile?: boolean) {
    ...
    try {
      conditions = JSON.parse(this.controlOpts.conditions).queryText
    } catch (error) {}
    // if (isMobile) {
    //
      ...
      .map(arr => {
        ...
        if (f2[0] === "{") {
          ...
          if (idx > -1) {
            ...
            const sheet = formControls.find(c => c.key === sheetKey) as any as typings.FormSheet
            if (sheet) {
              control = sheet.columns.find(c => c.key === controlKey) as any
            }

            const sheetCtrl = this.findController(f2.substring(0, idx)) as FormSheet
  ...
  getConfig(isMobile?: boolean) {
    ...
    return RelevanceFormControl.service.getListQueryConditions(schemaCode, queryCode, params)
  }

  open(paramsId?: any) {
    if (!this.ctrl.value || !this.ctrl.value.id || !RelevanceFormControl.service) {
      return
    }
  ...
  async getListQueryConditions(isMobile?: boolean) {
    ...
    let res: any = await RelevanceFormControl.service.getListQueryConditions(schemaCode, queryCode, params)
    console.log(res)
    console.log(1, this.control)
    try {
      // 如果配置了查询条件，使用配置的，如果没有就使用关联目标视图中配置的查询条件
      let conditions = JSON.parse(this.control.options.conditions)
  ...
  listenCommand() {
    this.commandSubscription = this.ctrl.commander.subscribe((cmd: ControlCommand) => {
      switch (cmd.name) {
        case "init":
          //#46646 只有新增时触发
          ...
          if (isNew) {
            ...
            if (this.controlOpts.defaultVal[0] === "[") {
              this.controlOpts.defaultVal = convertDataItemExp(this.controlOpts.defaultVal)
            }
            ...
            this.setDefaultValue(schemaCode, queryCode, cmd.args).then(results => {
              for (const result of results) {
                const sub = result.controller.valueChange.subscribe(() => {
                  ...
                  this.setDefaultValTimer = setTimeout(() => this.setDefaultValue(schemaCode, queryCode, cmd.args), 1000)
                })
  ...
  async setDefaultValue(schemaCode: string, queryCode: string, index: number = 0) {
    ...
    const _ctrl = this.ctrl as any
    const cols =
      _ctrl.options && _ctrl.options.mappings ? Object.keys(_ctrl.options.mappings).concat(this.control.options.displayField) : []
    if (RelevanceFormControl.service.queryDefaultValue) {
      const res = await RelevanceFormControl.service.queryDefaultValue(schemaCode, queryCode, cols, filters)
      ...
      if (res.data.length > 0) {
        ...
        if (_ctrl.inSheet) {
          ...
          if (sessionStorage.getItem("sheet-" + subSchemaCode)) {
            ...
            let arr: any = {}
            let mappings = _ctrl.options.mappings
            ...
            item = res.data[index]
              ? res.data[index]
              : res.data[0]
              ? await this.convertForMappings(res.data[index] ? res.data[index] : res.data[0])
              : {}
          }
        } else {
        ...
        // 关联表单 显示它默认字段.
        if (this.control.options.displayField !== "name" && !~Object.keys(item).indexOf(this.control.options.displayField)) {
  ...
  async setLinkageValue() {
    ...
    if (_ctrl.value && _ctrl.value.id) {
      ...
      if (!_ctrl.value.$format && (d.length || !(field in _ctrl.value) || _ctrl.value[field] === undefined)) {
        let value: any = null
        const dataitems = await RelevanceFormControl.service.dataitemsOf(options.schemaCode)
        ...
        if (errcode !== 0) {
          ...
          if (data && data.bizObject && data.bizObject.data) {
            ...
            // _ctrl.value = value;
            const formControls = this.getFormControls()
            let item = await ReverseQueryService.convertForMappings(value, dataitems, _ctrl.options.mappings, formControls)
  ...
  isFileToImage(key: string, relevanceKey: string, value: any) {
    ...
    const showCtrl: RendererFormControl | undefined = this.getFormControls().find((ctrl: RendererFormControl) => ctrl.key === key)
      ...
      try {
        if (val instanceof Date) {
          ...
          if (val.length === 1) {
            ...
            return val.reduce((c, item) => `${c}、${item.name ? item.name : item}`, "").slice(1)
          }
        } else if (
          val &&
      ...
      try {
        ...
        if (typeof a === "object") {
          ...
          if (field === "createdTime") {
            ...
            return common.utils.DateHandle.dateFormat(new Date(theDate), "YYYY-MM-DD HH:mm:ss")
          }
          return val
        }
      } catch (e) {
...
export interface RelevanceFormService {
   ...
   * @param value  控件值
   */
  open(schemaCode: string, queryCode: string, id: string, sheetId: string, isAuthorize: boolean, code: string, value: any): void

  dataitemsOf(schemaCode: string): Promise<typings.DataItemLike[]>

  getBizmodelTitle(schemaCode: string): Promise<any | null>

  getListQueryConditions?(schemaCode: string, listCode: string, params: RelevanceFormSearchParams[]): Promise<any | null>
  ...
  queryDefaultValue?(schemaCode: string, queryCode: string, columns: string[], filters: any[]): Promise<Segment<RelevanceFormSearchResult>>

  scan(successFn: (data: string) => void, failFn?: Function): void

  uploadImageFromCamera(successFn: (data: string[]) => void, failFn?: Function): void
