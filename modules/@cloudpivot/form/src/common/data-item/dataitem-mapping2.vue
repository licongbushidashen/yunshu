...
<template>
  <div :style="customStyle ? customStyle.bgStyle : ''">
    <a-row type="flex" class="header">
      ...
      <a-col class="col" :span="2" v-if="editable"></a-col>
    </a-row>
    ...
    <a-row type="flex" v-for="(item,index) in selected" :key="index">
      ...
      <a-col class="col" :span="2" v-if="editable">
  ...
  findSourceOptionAndDisabled(code: string, disabled: boolean) {
    ...
    this.sourceOptions=this.sourceOptions.map(item=>{
      if(item.value === code){
        ...
        item.disabled= this.selected.findIndex(item2=>item2.source.code===item.value)>-1;
      }
      return item
    })
